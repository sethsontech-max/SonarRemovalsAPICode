...
def append_inactive_reason(row):
    if row["Account_account_status_id"] not in [1, 11]:
        if row["reason"] is None or row["reason"] == "":
            return "inventory assigned to Inactive account"
        return row["reason"] + "; inventory assigned to Inactive account"
    return row["reason"]
...
#now we need to create list of removals by combining all three lists and deduplicating by inventory ID
    ...
    # find all rows in which there is no account_id_inv and mark them as 'no account' in reason column
